...
old_order = []
new_order = []
excel = load_workbook('./data/淘宝发货.xlsx')
...
if old_order:
    # 实例化
    filepath = f'./excel/淘宝发货_老店_{time.strftime("%Y%m%d%H%M%S", time.localtime(time.time()))}.xlsx'
...
if new_order:
    # 实例化
    filepath = f'./excel/淘宝发货_新店_{time.strftime("%Y%m%d%H%M%S", time.localtime(time.time()))}.xlsx'
    taobao = Taobao(filepath=filepath)
    driver = taobao.broswer()
    #初始化excel
    taobao.initexcel()
