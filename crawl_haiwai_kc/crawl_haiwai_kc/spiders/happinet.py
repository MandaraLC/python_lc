...
class HappinetSpider(scrapy.Spider):
    name = 'happinet'
    allowed_domains = ['happinetonline.com']
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(HappinetSpider, self).__init__(*args, **kwargs)
        """
        获取命令行参数
        """
        # 加载必要参数
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(HappinetSpider, cls).from_crawler(
            crawler, *args, **kwargs)
    ...
    def parse(self, response):
        ...
        item = OutputItem()
        # # 商品名称
        goods_name = response.xpath('//h1[@class="maintitle"]/text()').extract_first()
        ...
        buy_icon = response.xpath('//div[@id="cartOn"]/@style')
        quantity = response.xpath('//div[@class="quantity clearfix"]')

        if not buy_icon:
            item["goods_status"] = -1  # 未知错误
        elif buy_icon and quantity:
