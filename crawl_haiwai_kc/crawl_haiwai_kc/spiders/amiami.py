...
class AmiamiSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(AmiamiSpider, self).__init__(*args, **kwargs)
        """
        获取命令行参数
        """
        # 加载必要参数
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(AmiamiSpider, cls).from_crawler(
            crawler, *args, **kwargs)
    ...
    def parse(self, response):
        ...
        item = OutputItem()
        # # 商品名称
        goods_name = response.xpath('//div[@class="title_area"]/h2[@class="heading_10"]/text()').extract_first()
        ...
        icon = response.xpath('//div[@class="cartbtn"]//div[contains(@class, preorder)]/@class').extract_first()

        if not icon:
            item["goods_status"] = -1  # 未知错误
        elif icon == 'cartbtn_preorder':
