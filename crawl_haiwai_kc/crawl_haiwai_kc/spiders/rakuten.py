...
class RakutenSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        #     'crawl_haiwai_kc.middlewares.RandomDelayDownloaderMiddleware': 200,
        # },
    }
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(RakutenSpider, cls).from_crawler(
            crawler, *args, **kwargs)
    ...
    def parse(self, response):
        ...
        goods_name = response.css(
            'span.item_name>b::text').extract_first() or None
        # 购买按钮
        btnClass = response.css(
            'button[data-ratid="fixed_checkout"]::attr("class")').extract_first() or ""
        # 没有库存msg
        sold_out = response.css('.soldout_msg').extract_first()
        ...
        """ 
            ...
            1、判断按钮的class类名
                包含 checkout ---- 有库存
            2、判断是否有class类名 'soldout_msg' 的标签
                有标签则是没有库存
            3、以上都不符合则返回-1
        """
        # 判断是否可购买
        if 'checkout' in btnClass:
